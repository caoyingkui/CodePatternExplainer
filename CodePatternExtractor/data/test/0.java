...
public class MergeCellTest {
    ...
    private static void firstExcel() throws IOException {
        HSSFWorkbook wb = new HSSFWorkbook();

        HSSFSheet sheet = wb.createSheet("first excel");
        sheet.setDefaultColumnWidth((short) 18);

        HSSFCellStyle headerCellStyle = createHeaderStyle(wb);
        HSSFRow headerRow = createHeaderRow(sheet, headerCellStyle,
                new String[] { "订单ID", "下单用户", "下单时间" });

        wb.write(new File("D:\\order.xls"));
        wb.close();
    }
    ...
    private static HSSFRow createHeaderRow(HSSFSheet sheet,
                                           HSSFCellStyle headerCellStyle, String[] headers) {
    ...
    private static void createComplexExcel() throws IOException {
